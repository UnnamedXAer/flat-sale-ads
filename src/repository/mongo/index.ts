...
class MongoRepository implements IRepository {
  ...
  async getTmpOffers(): Promise<IOffersInfo[]> {
    const offersInfoDocs = await this.models.tmpOffers.find({}).sort({ created_at: -1 });
  ...
  async connect(uri: string, options?: ConnectionOptions) {
...
export const connectToStorage: ConnectToStorage = async (logger: Logger) => {
  const storage = new MongoRepository(logger, {
  ...
  await storage.connect(
    ...
    connectOptions as ConnectionOptions
  );
